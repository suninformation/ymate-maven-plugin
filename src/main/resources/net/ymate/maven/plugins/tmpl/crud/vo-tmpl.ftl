...
@QOrderBy({<#list api.query.orderFields as orderField>@QOrderField(prefix = "${orderField.prefix!""}", value = <#if (orderField.value!"")?contains(".")>${orderField.value!""}<#else>"${orderField.value!""}"</#if>, type = QOrderField.Type.DESC)<#if orderField_has_next>, </#if></#list>})</#if></#if>
