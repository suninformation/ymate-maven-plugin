...
<#macro parseField p array><#if apidocs>@ApiParam
                         </#if>@VRequired<#if p.config?? && p.config.query?? && p.config.query.validation?? && p.config.query.validation.length?? && p.config.query.validation.length.enabled><#if p.config.query.validation.dateTime?? && p.config.query.validation.dateTime.enabled><#else>
                         @VLength(min = ${p.config.query.validation.length.min}, max = ${p.config.query.validation.length.max}, eq = ${p.config.query.validation.length.eq}<#if p.config.query.validation.length.msg?? && (p.config.query.validation.length.msg?length > 0)>, msg = "${(p.config.query.validation.length.msg)!""}"</#if>)</#if></#if><#if p.config?? && p.config.query?? && p.config.query.validation?? && p.config.query.validation.dataRange?? && p.config.query.validation.dataRange.enabled && p.config.query.validation.dataRange.values?? && (p.config.query.validation.dataRange.values?size > 0)>
                         @VDataRange(value = {<#list p.config.query.validation.dataRange.values as v>"${v}"<#if v_has_next>, </#if></#list>}, ignoreCase = ${p.config.query.validation.dataRange.ignoreCase?string}<#if p.config.query.validation.dataRange.msg?? && (p.config.query.validation.dataRange.msg?length > 0)>, msg = "${(p.config.query.validation.dataRange.msg)!""}"</#if>)</#if><#if p.config?? && p.config.query?? && p.config.query.validation??><#if p.config.query.validation.regex?? && p.config.query.validation.regex.enabled && p.config.query.validation.regex.regex?? && (p.config.query.validation.regex.regex?length > 0)>
                         @VRegex(regex = "${p.config.query.validation.regex.regex}"<#if p.config.query.validation.regex.msg?? && (p.config.query.validation.regex.msg?length > 0)>, msg = "${(p.config.query.validation.regex.msg)!""}"</#if>)</#if><#if p.config.query.validation.idCard?? && p.config.query.validation.idCard.enabled>
                         @VIDCard<#if p.config.query.validation.idCard.msg?? && (p.config.query.validation.idCard.msg?length > 0)>(msg = "${(p.config.query.validation.idCard.msg)!""}")</#if></#if><#if p.config.query.validation.email?? && p.config.query.validation.email.enabled>
                         @VEmail<#if p.config.query.validation.email.msg?? && (p.config.query.validation.email.msg?length > 0)>(msg = "${(p.config.query.validation.email.msg)!""}")</#if></#if><#if p.config.query.validation.mobile?? && p.config.query.validation.mobile.enabled>
                         @VMobile(regex = "${p.config.query.validation.mobile.regex!""}"<#if p.config.query.validation.mobile.msg?? && (p.config.query.validation.mobile.msg?length > 0)>, msg = "${(p.config.query.validation.mobile.msg)!""}"</#if><#if p.config.query.validation.mobile.msg?? && (p.config.query.validation.mobile.msg?length > 0)>, msg = "${(p.config.query.validation.mobile.msg)!""}"</#if>)</#if><#if p.config.query.validation.numeric?? && p.config.query.validation.numeric.enabled>
                         @VNumeric(min = ${p.config.query.validation.numeric.min}, max = ${p.config.query.validation.numeric.max}, eq = ${p.config.query.validation.numeric.eq}, decimals = ${p.config.query.validation.numeric.decimals}<#if p.config.query.validation.numeric.msg?? && (p.config.query.validation.numeric.msg?length > 0)>, msg = "${(p.config.query.validation.numeric.msg)!""}"</#if>)</#if></#if><#if p.description?? && (p.description?length > 0)>
...
import ${app.packageName}.bean.impl.*;
...
import ${app.packageName}.vo.impl.${prefix}${api.name?cap_first}VO;<#if apidocs>
 ...
 *
 * ${prefix}${api.name?cap_first}Controller generated By CrudMojo
 *
...
public class ${prefix}${api.name?cap_first}Controller {

    @Inject
    private JDBC database;

    @Inject
    private I${prefix}${api.name?cap_first}Repository repository;

    <#if !(api.settings??) || api.settings.enableQuery!true><#if apidocs>@ApiAction(value = "${languageMap.query}", description = "")
    @ApiResponses(description = "", type = ${prefix}${api.name?cap_first}VO[].class)
    @ApiGenerateResponseExample(paging = true)</#if>
    @RequestMapping("/query")
    public Object query(<#if apidocs>@ApiParam </#if>@VModel @ModelBind ${prefix}${api.name?cap_first}DTO ${api.name?uncap_first},

                        <#if apidocs>@ApiParam </#if>@VModel @ModelBind PageDTO page) throws Exception {
        IResultSet<I${prefix}${api.name?cap_first}VO> resultSet = repository.query${api.name?cap_first}s(database, ${api.name?uncap_first}.toBean(), <#if hideInListFields?? && (hideInListFields?size > 0)>Fields.create(<#list hideInListFields as field><@buildFieldName field, false/><#if field_has_next>, </#if></#list>), </#if>page.toPage());
    ...
    public Object detail(<#list primaryFields as p><@parseField p false/><#if p_has_next>,
        ...
        I${prefix}${api.name?cap_first}VO ${api.name?uncap_first} = repository.query${api.name?cap_first}(database, <#if multiPrimaryKey>${prefix}${api.name?cap_first}Repository.buildPrimaryKey(<#list primaryFields as p>${p.name}<#if p_has_next>, </#if></#list>)<#else>${primaryKey.name}</#if>);
        if (${api.name?uncap_first} != null) {
            return WebResult.builder().succeed().data(${api.name?uncap_first});
        }
    ...
    public Object update(<#list primaryFields as p><@parseField p false/><#if p_has_next>,
                         ...
                         <#if apidocs>@ApiParam </#if>@VModel @ModelBind ${prefix}${api.name?cap_first}UpdateDTO ${api.name?uncap_first}Update<#if lastModifyTimeProp?? && !lastModifyTimeProp.foreign>,

                         <@parseField lastModifyTimeProp false/></#if>) throws Exception {
        ErrorCode result = repository.update${api.name?cap_first}(database, <#if multiPrimaryKey>${prefix}${api.name?cap_first}Repository.buildPrimaryKey(<#list primaryFields as p>${p.name}<#if p_has_next>, </#if></#list>)<#else>${primaryKey.name}</#if>, ${api.name?uncap_first}Update.toBean()<#if lastModifyTimeProp?? && !lastModifyTimeProp.foreign>, ${lastModifyTimeProp.name}</#if>);
        if (result != null) {
            return WebResult.builder(result);
        }
        return WebResult.builder(WebErrorCode.resourceNotFoundOrNotExist());
    ...
    public Object export(<#if apidocs>@ApiParam </#if>@VModel @ModelBind ${prefix}${api.name?cap_first}DTO ${api.name?uncap_first}) throws Exception {
        File resultFile =  ExcelFileExportHelper.bind(index -> {
            IResultSet<I${prefix}${api.name?cap_first}VO> resultSet = repository.query${api.name?cap_first}s(database, ${api.name?uncap_first}.toBean(), <#if notExportFields?? && (notExportFields?size > 0)>Fields.create(<#list notExportFields as field><@buildFieldName field true/><#if field_has_next>, </#if></#list>), </#if>Page.create(index).pageSize(10000).count(false));
            if (resultSet != null && resultSet.isResultsAvailable()) {
                return resultSet.getResultData();
            }
            return null;
        }).export(${prefix}${api.name?cap_first}VO.class);
        if (resultFile != null) {
